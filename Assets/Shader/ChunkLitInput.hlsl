...
//float4 _CoastlineTex_ST;
//float4 _HillTex_ST;
float4 _HillColor;
float4 _CoastlineColor;
float _Boundaries;
...
//TEXTURE2D(_CoastlineTex);     SAMPLER(sampler_CoastlineTex);
//TEXTURE2D(_HillTex);          SAMPLER(sampler_HillTex);
...
float3 ApplyDetailNormal(float2 detailUv, half3 normalTS, half detailMask)
{
#if defined(_DETAIL)
...
inline void InitializeStandardLitSurfaceData(float2 uv, out SurfaceData outSurfaceData, float3 positionWS, float3 normalWS)
{
    //πŸ¿Ãø»
    float t = clamp(positionWS.y - _Boundaries, 0, 1);
    t = smoothstep(0, 1, t);

    //float2 biomeTexUV = positionWS.xz;
    //float4 albedoAlpha = lerp(SampleAlbedoAlpha(biomeTexUV, TEXTURE2D_ARGS(_HillTex, sampler_HillTex)), SampleAlbedoAlpha(biomeTexUV, TEXTURE2D_ARGS(_CoastlineTex, sampler_CoastlineTex)), t);
    float4 albedoAlpha = lerp(_CoastlineColor, _HillColor, t);
